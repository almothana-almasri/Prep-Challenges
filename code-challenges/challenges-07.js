...
const objLat = (obj) => {
    ...
    let goodFirstName = firstName.charAt(0).toUpperCase() + firstName.slice(1);
    let goodLastName = lastName.charAt(0).toUpperCase() + lastName.slice(1);
    return `my name is ${goodFirstName} ${goodLastName} I am ${age} YO, and I love ${hobby}.`;
...
const cvFormatter = (arr) => {
    const newArr = [];
    let fullName = ""
    // for (let i in arr) { // we can use this method as teacher Rania hinted
    //     const obj = arr[i];
    //     const { firstName, lastName, yearsOfExperience, tech } = obj;
    for (let i = 0; i < arr.length; i++) { // or we can use this one
        const { firstName, lastName, tech, yearsOfExperience } = arr[i];
